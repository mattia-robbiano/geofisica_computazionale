...
CONTAINS
   SUBROUTINE OPEN_INPUT_FILE(FILE_UNIT, FILE_NAME, ERROR)
      ...
      IF (.NOT. iexist) THEN
         ! File does not exist
         WRITE(*, 101) 'Error: file not found: ', TRIM(FILE_NAME)
         ERROR = .TRUE.
      ELSE
         ! File exists, open it
         ...
         IF (IO /= 0) THEN
            ...
            ERROR = .TRUE.
         ELSE
            WRITE(*,101) 'File opened successfully: ', TRIM(FILE_NAME)
            ERROR = .FALSE.
         END IF
      END IF
      101   FORMAT(A, 1X, A)
   ...
   SUBROUTINE OPEN_OUTPUT_FILE(FILE_UNIT, FILE_NAME, ERROR)
      ...
      IF (iexist) THEN
         ...
         WRITE(*,101) 'File already exists: ', TRIM(FILE_NAME)
         ...
         IF (response == 'Y' .OR. response == 'y') THEN
            ...
            IF (IO /= 0) THEN
               ...
               ERROR = .TRUE.
            ELSE
               WRITE(*,101) 'File replaced and opened sucessfully: ', TRIM(FILE_NAME)
               ERROR = .FALSE.
            END IF
         ELSE
            WRITE(*,101) 'User chose not to replace the file: ', TRIM(FILE_NAME)
            ERROR = .TRUE.
         END IF
      ELSE
         ...
         IF (IO /= 0) THEN
            ...
            ERROR = .TRUE.
         ELSE
            WRITE(*,101) 'File created and opened sucessfully: ', TRIM(FILE_NAME)
            ERROR = .FALSE.
         END IF
      END IF
      101   FORMAT(A, 1X, A)
